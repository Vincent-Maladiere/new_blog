...
const Hero = () => {
  return (
    <section className="flex flex-col justify-between gap-10">
      <div className="flex flex-row gap-5 item-start conf-card">
        <Image
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="w-full flex justify-center items-center align-center gap-[20%]">
          {libs.map((l) => (
            <Link key={l.title} href={l.url}>
              <Image alt={l.title} src={l.img} width={100} height={100} />
            </Link>
          ))}
        </div>
      </div>
